...
parser ParserImpl(packet_in packet, out headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".start") state start {
        ...
        transition select((packet.lookahead<bit<48>>())[47:0]) {
            48w0: parse_cpu_header;
...
control egress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".do_rewrites_no_cpu") action do_rewrites_no_cpu(bit<48> smac) {
        hdr.ethernet.srcAddr = smac;
        hdr.ipv4.srcAddr = meta.meta.ipv4_sa;
        hdr.ipv4.dstAddr = meta.meta.ipv4_da;
        hdr.tcp.srcPort = meta.meta.tcp_sp;
    ...
    @name(".send_frame_no_cpu") table send_frame_no_cpu {
        actions = {
            do_rewrites_no_cpu;
            _drop;
        }
